...
class AuthRepository {
    ...
    suspend fun authenticate(username: String, password: String): UserDto? = dbQuery {
        ...
        if (user != null && BCrypt.checkpw(password, user.second)) {
            UserDto(
                ...
                email = user.first[Users.email],
                emailVerified = user.first[Users.emailVerified],
    ...
    suspend fun register(request: RegisterRequest): AuthResponse? = dbQuery {
        ...
        val existingEmail = Users
            .select { Users.email eq request.email }
            .singleOrNull()
        ...
        val userId = Users.insert {
            ...
            it[emailVerified] = false  // Will be verified via email
        ...
        generateAuthResponse(userId.value.toString(), request.username, request.email, false, request.displayName)
    }
        ...
        val user = Users
            ...
            .map {
                listOf(
                    ...
                    it[Users.username],
                    it[Users.email],
                    it[Users.emailVerified].toString(),
                    it[Users.displayName]
                )
            }
            .firstOrNull() ?: return@dbQuery null
        
        generateAuthResponse(user[0], user[1], user[2], user[3].toBoolean(), user[4])
    }
