...
object Validator {
    ...
    fun validateEmail(email: String): ValidationResult {
        return when {
            ...
            email.length > 255 -> ValidationResult.Error("Email must be less than 255 characters")
            else -> ValidationResult.Success
        }
    }
...
fun Route.authRoutes(
    ...
    route("/auth") {
        ...
        post("/register") {
            ...
            when (val result = Validator.validateEmail(request.email)) {
                is Validator.ValidationResult.Error -> errors["email"] = result.message
                else -> {}
            }
            ...
            if (userRepository.getUserByEmail(request.email) != null) {
                throw ValidationException(
                    "Registration failed",
                    mapOf("email" to "Email already exists")
                )
            }
        ...
        post("/refresh") {
            ...
            val authResponse = authRepository.refreshToken(request.refreshToken)
